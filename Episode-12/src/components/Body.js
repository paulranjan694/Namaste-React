import ResturantCard, { withDiscount } from "./ResturantCard";
...
const Body = () => {
  ...
  const WithDiscountResturantCard = withDiscount(ResturantCard);
    ...
    <div className="body">
      <div className="flex">
        <div className="search m-4 p-4">
          ...
          />
          <button
            className="bg-gray-300 px-4 py-1 m-4 rounded-lg"
            onClick={() => {
          ...
          <label>Username for context api demo - </label>
          <input
            type="text"
            className="border border-black border-solid"
            value={loggedInUser}
      ...
      <div className="flex flex-wrap">
        {filteredResturants.map((resturant) => (
          <Link key={resturant.info.id} to={"/resturants/" + resturant.info.id}>
            ...
            Object.keys(resturant.info.aggregatedDiscountInfoV3).length > 0 ? (
              <WithDiscountResturantCard resData={resturant} />
            ) : (
